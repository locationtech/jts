...
import org.locationtech.jts.geom.impl.CoordinateArrayScannerSequence;
...
public class Orientation {
   ...
   */
  public static boolean NEWisCCW(Coordinate[] ring)
  {
    ...
    /**
     * Find first high point after a lower point, if one exists
     ...
     */
    Coordinate prevPt = ring[0];
    Coordinate upHiPt = prevPt;
    Coordinate upLowPt = null;
    int iUpHi = 0;
    for (int i = 1; i <= nPts; i++) {
      Coordinate p = ring[i];
      /**
       * If segment is upwards and hi point is higher, record it
       */
      if (p.y > prevPt.y 
          && p.y >= upHiPt.y) {
        upHiPt = p;
    ...
    do {
      iLow = (iLow + 1) % nPts;
    } while (iLow != iUpHi && ring[iLow].y == upHiPt.y );

    Coordinate downLowPt = ring[iLow];
    int iDownHi = iLow - 1;
    if (iDownHi < 0) iDownHi = nPts - 1;
    Coordinate downHiPt = ring[iDownHi];
  ...
  {
    return isCCW(new CoordinateArrayScannerSequence(ring));
  }
   ...
   */
  public static boolean OLDisCCW(CoordinateSequence ring)
  {
    // # of points without closing endpoint
    int nPts = ring.size() - 1;
    // sanity check
