...
implements SpatialIndex<T>, Serializable
{
   ...
   */
  public T[] nearestNeighbour(STRtree<T> tree, ItemDistance<T,Envelope> itemDist)
  {
    if (isEmpty() || tree.isEmpty()) return null;
    BoundablePair<T,Envelope> bp = new BoundablePair<>(this.getRoot(), tree.getRoot(), itemDist);
    return nearestNeighbour(bp);
  }
   ...
   */
  public boolean isWithinDistance(STRtree<T> tree, ItemDistance<T,Envelope> itemDist, double maxDistance)
  {
    BoundablePair<T,Envelope> bp = new BoundablePair<>(this.getRoot(), tree.getRoot(), itemDist);
   ...
   */
  public T[] nearestNeighbour(Envelope env, Object item, ItemDistance<T,Envelope> itemDist,int k)
  {
    Boundable<Envelope> bnd = new ItemBoundable<>(env, item);
    BoundablePair<T,Envelope> bp = new BoundablePair<>(this.getRoot(), bnd, itemDist);
