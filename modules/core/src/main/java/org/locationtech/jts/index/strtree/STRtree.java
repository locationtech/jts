...
implements SpatialIndex<T>, Serializable
{
  ...
  private static final class STRtreeNode extends AbstractNode<Envelope>
  {
    ...
    protected Envelope computeBounds() {
      ...
      for (Boundable<Envelope> childBoundable : getChildBoundables()) {
        if (bounds == null) {
   ...
   * a new (parent) node.
   */
  protected List<AbstractNode<Envelope>> createParentBoundables(List<Boundable<Envelope>> childBoundables, int newLevel) {
  ...
  private List<AbstractNode<Envelope>> createParentBoundablesFromVerticalSlices(List<Boundable<Envelope>>[] verticalSlices, int newLevel) {
    Assert.isTrue(verticalSlices.length > 0);
    List<AbstractNode<Envelope>> parentBoundables = new ArrayList<>();
    for (List<Boundable<Envelope>> verticalSlice : verticalSlices) {
  ...
  protected List<AbstractNode<Envelope>> createParentBoundablesFromVerticalSlice(List<Boundable<Envelope>> childBoundables, int newLevel) {
   ...
   */
  public T[] nearestNeighbour(ItemDistance<T> itemDist)
  {
   ...
   *    or <code>null</code> if the tree is empty
   */
  public T nearestNeighbour(Envelope env, T item, ItemDistance<T> itemDist)
  {
   ...
   */
  public T[] nearestNeighbour(STRtree<T> tree, ItemDistance<T> itemDist)
  {
  ...
  private T[] nearestNeighbour(BoundablePair initBndPair)
  {
   ...
   */
  public boolean isWithinDistance(STRtree<?> tree, ItemDistance<T> itemDist, double maxDistance)
  {
    BoundablePair bp = new BoundablePair(this.getRoot(), tree.getRoot(), itemDist);
   ...
   */
  public T[] nearestNeighbour(Envelope env, T item, ItemDistance<T> itemDist,int k)
  {
    Boundable<Envelope> bnd = new ItemBoundable<>(env, item);
    BoundablePair bp = new BoundablePair(this.getRoot(), bnd, itemDist);
  ...
  private T[] nearestNeighbourK(BoundablePair initBndPair, int k)
  {
    return nearestNeighbourK(initBndPair, Double.POSITIVE_INFINITY,k);
  }
  
  private T[] nearestNeighbourK(BoundablePair initBndPair, double maxDistance, int k)
  {
  ...
  private static <T>T[] getItems(PriorityQueue<BoundablePair> kNearestNeighbors)
  {
	  /** 
