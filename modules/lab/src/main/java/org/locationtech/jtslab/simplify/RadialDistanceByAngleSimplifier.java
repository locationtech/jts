...
import org.locationtech.jts.geom.Lineal;
    ...
    public class RadialDistanceByAngleSimplifier
    {
        ...
        private static Coordinate TransferCoordinate(
        ...
        private LineString simplify(LineString input)
        {
          ...
          Coordinate c1 = TransferCoordinate(inputSeq, 0, outputSeq, 0);
          Coordinate c2 = TransferCoordinate(inputSeq, 1, outputSeq, 1);
          int outputIndex = 2;
          for (int i = 2; i < inputSeq.size() - 1; i++) {
            ...
            switch (simplifyByAngleDistance(c1, c2, current, future))
            {
              case -1: // ignore current 
                break;
              case 0:  // update last
                c2 = TransferCoordinate(inputSeq, i, outputSeq, outputIndex-1);
                break;
              case 1:  // is valid
                c1 = c2;
                c2 = TransferCoordinate(inputSeq, i, outputSeq, outputIndex++);;
                break;
            }
          }

          // add last coordinate, no matter what
          TransferCoordinate(inputSeq, inputSeq.size()-1, outputSeq, outputIndex++);;
